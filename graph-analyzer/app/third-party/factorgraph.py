...
RV vars:
    X       the set of n random variables   n个随机变量的集合
    X_i     random variable i (1 <= i <= n) 随机变量i（1<=i<=n）
    v_i     number of values that X_i can take (nonstandard but I wanted one)   我可以接受的X_值的数目（非标准，但我想要一个）
    x_ij    a particular value for X_i (1 <= j <= v_i)  X_i的一个特殊值（1<=j<=v_i）
    x_i     a simpler (lazy) notation for x_ij (which j doesn't matter) 一个简单的（懒惰的）x_ij表示法（j不重要）
    x       a set of x_i for i = 1..n (imagine x_1j, x_2k, ..., x_nz)   i=1..n的一组x_i（想象x_1j，x_2k，…，x_nz）

Factor vars:
    F       the set of m factors    m因子集
    f_a     factor a (1 <= a <= m) connecting a subset of X  因子a（1<=a<=m）连接X的子集
    X_a     the subset of X (RVs) that f_a connects     f_a连接的X（RVs）的子集
    x_a     the subset of x (values for RVs) that f_a connects      f_a连接的x（RVs值）的子集

Functions:
    p(x)    joint distribution for p(X = x)     p（X=x）的联合分布
...
# convenience.
# 使用此选项可打开或关闭所有调试。预期用途：当你试试看。一旦你知道这些东西管用，就立刻关掉。
# 也可以在创建每个实例时指定，但此全局开关是为 方便
DEBUG_DEFAULT = True

# This is the maximum number of iterations that we let loopy belief propagation
# run before cutting it off.这是我们在切断循环信念传播之前让其运行的最大迭代次数。
...
# Let the user Ctrl-C at any time to stop.  让用户随时按Ctrl-C键停止。
...
def divide_safezero(a, b):
    '''
    Divies a by b, then turns nans and infs into 0, so all division by 0  becomes 0.
    将a除以b，然后将nans和infs变成0，因此所有除以0的部分都变成0。
...
class Graph(object):
    ...
    functionality might be stuffed in here later.现在的图表没有意义，真的（除了记录所有的RVs和
    因素，假设我们记得加上它们），所以这可能会被删除或稍后可能会在这里填充功能。
    ...
    def factor(self, rvs, name='', potential=None, meta={},
               debug=DEBUG_DEFAULT):
        '''
        创建一个因子，将其添加到此图中，然后返回它。方便功能。
        请注意，如果您愿意，可以提供RV的名称而不是RV。
        参数：
        随机变量（[RV | str]）
        名称（str，opt）
        潜力(np.数组，选项）
        ...
        for i in range(len(rvs)):
            if debug:
                assert type(rvs[i]) in [str, UnicodeError, RV]
            if type(rvs[i]) in [str, UnicodeError]:
    ...
    def lbp(self, init=True, normalize=False, max_iters=1, progress=False):
        '''
...
class RV(object):
    ...
    def __init__(self, name, n_opts, labels=[], meta={}, debug=DEBUG_DEFAULT):
        ...
        if debug:
            ...
            for l in labels:
                assert type(l) in [str, UnicodeError]
    ...
    def has_label(self, label):
        ...
        if len(self.labels) == 0:
            ...
            if self.debug:
                assert type(label) in [int, str, UnicodeError]
            if type(label) in [str, UnicodeError]:
...
class Factor(object):
    ...
    def set_potential(self, p):
        ...
        conceptualizing（概念化） it. What gives? Well, what we're doing is indexing in
        the correct order. So, the first dimension（维）, 3, indexes the value for
        the variable A. This visually（视觉上） splits our table into three areas, one
